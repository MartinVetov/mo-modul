...
function summary_data(int $methodistId, int $yid, string $term): array
{
    $P = [':m' => $methodistId, ':y' => $yid, ':t' => $term];
    ...
    $sum = one('SELECT COUNT(*) n, COUNT(DISTINCT user_id) teachers, AVG(avg_grade) avg,
                ...
                FROM mo_v_entries
                WHERE methodist_id=:m AND status="sent" AND year_id=:y AND term=:t', $P)
...
function ai_context(array $D, string $methodistName, string $term): array
{
    ...
    return [
        'department' => 'МО с методист ' . $methodistName,
        'year' => $D['year'],
