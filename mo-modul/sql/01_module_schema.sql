...
-- ---------------------------------------------------------------------
-- Методически обединения. Председателят и заместникът получават
-- анализите по всички предмети на своето МО.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Предмети. Всеки предмет принадлежи на едно МО – оттам се разбира
-- при кого отива анализът.
-- ---------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS mo_competencies (
  ...
  section     VARCHAR(120) NULL,             -- подраздел; ползва се само при БЕЛ
  ...
  INDEX idx_mc_section (subject_id, grade_level, section)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ---------------------------------------------------------------------
-- Съответствия между имената на предметите във файловете и предмета
-- в системата. Пример: „Литература“ отива в „Български език и
-- литература“ с подраздел „Литература“, вместо да се създава нов предмет.
-- ---------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS mo_teacher_methodist (
  ...
  methodist_id  BIGINT UNSIGNED NULL,         -- кой е писал (за история)
...
CREATE TABLE IF NOT EXISTS mo_entries (
  ...
  methodist_id  BIGINT UNSIGNED NULL,         -- (не се ползва след v5)
  department_id INT NULL,                     -- МО, което получава анализа
  ...
  INDEX idx_ent_dep (department_id, status, year_id, term)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS mo_summaries (
  ...
  status       ENUM('draft','sent') NOT NULL DEFAULT 'draft',
  deputy_id     BIGINT UNSIGNED NULL,
  finalized_by  BIGINT UNSIGNED NULL,         -- председател или заместник
  sent_at       DATETIME NULL,
  updated_at   TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
