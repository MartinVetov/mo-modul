...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    /* При надвишен post_max_size PHP изпразва $_POST и $_FILES – без тази
       проверка излиза подвеждащото „Изтекла сесия“. */
    if (empty($_POST) && empty($_FILES) && (int)($_SERVER['CONTENT_LENGTH'] ?? 0) > 0) {
        flash('Общият размер на файловете надхвърля разрешеното от PHP ('
            . e(ini_get('post_max_size')) . '). Качете ги на по-малки групи или увеличете '
            . 'post_max_size и upload_max_filesize в php.ini.', 'err');
        redirect(base_url('pages/admin_competencies.php'));
    }
    csrf_check();
    $act = $_POST['action'] ?? '';

    /* ---------------- импорт от таблици (един или няколко файла) ---------------- */
    if ($act === 'import' && isset($_FILES['files'])) {
        $dry     = !empty($_POST['dry_run']);
        $replace = !empty($_POST['replace']);
        try {
            /* --------------------------------------------------------------
             * Професиите/специалностите се избират в страницата, не се четат
             * от файловете. Важат за ВСИЧКИ качени файлове.
             * ------------------------------------------------------------ */
            ...
            /* подредба на качените файлове в удобен за обхождане вид */
            $uploaded = [];
            foreach ((array)($_FILES['files']['name'] ?? []) as $i => $name) {
                ...
                $uploaded[] = [
                    ...
                    'error' => (int)($_FILES['files']['error'][$i] ?? 1),
                ];
            }
            if (!$uploaded) throw new RuntimeException('Не е избран нито един файл.');

            /* Подразделът важи само за БЕЛ. Ако файлът сам се назовава
               „Литература“ или „Български език“, той има превес. */
            $sectionPick = trim((string)($_POST['section'] ?? ''));
            if (!in_array($sectionPick, BEL_SECTIONS, true)) $sectionPick = '';

            $log = ['ok' => 0, 'skip' => 0, 'dupes' => 0, 'rows' => 0, 'subjects' => [],
                    'errors' => [], 'pairs' => [], 'files' => [], 'failed' => [],
                    'targets' => array_column($targets, 'label')];

            /* подредбата продължава между файловете, за да не се преплитат */
            $order = [];
            $wiped = [];
            ...
            foreach ($uploaded as $uf) {
                $fileLog = ['name' => $uf['name'], 'sheet' => '', 'head' => 0,
                            'rows' => 0, 'ok' => 0, 'skip' => 0, 'dupes' => 0];
                try {
                    if ($uf['error'] !== UPLOAD_ERR_OK) {
                        throw new RuntimeException($uf['error'] === UPLOAD_ERR_INI_SIZE
                            ? 'файлът е по-голям от разрешеното в php.ini'
                            : 'файлът не е качен (код ' . $uf['error'] . ')');
                    }

                    $ext    = strtolower(pathinfo($uf['name'], PATHINFO_EXTENSION));
                    $isXlsx = $ext === 'xlsx';
                    $sheets = $isXlsx ? SimpleXlsx::sheetNames($uf['tmp']) : [''];
                    if (!$sheets) $sheets = [0];

                    $rows = []; $head = -1; $sheet = '';
                    foreach ($sheets as $i => $name) {
                        $cand = SimpleXlsx::rows($uf['tmp'], $isXlsx ? $i : 0, $ext);
                    ...
                    if ($head < 0) {
                        throw new RuntimeException('липсва заглавен ред с колони „Клас“, „Предмет“ и „Компетенция“');
                    }
                    ...
                    foreach (array_slice($rows, $head + 1) as $ln => $r) {
                        ...
                        if ($level < 1 || $level > 12 || $subj === '' || $title === '') {
                            $log['errors'][] = $uf['name'] . ', ред ' . $no . ': непълни данни (клас/предмет/компетенция).';
                            $log['skip']++; $fileLog['skip']++;
                            continue;
                        }

                        /* „Литература“ и „Български език“ не стават отделни предмети,
                           а подраздели на „Български език и литература“. */
                        [$subjName, $section] = normalize_subject($subj);
                        if ($subjName === BEL_SUBJECT && $section === null && $sectionPick !== '') {
                            $section = $sectionPick;
                        }
                        if ($subjName !== BEL_SUBJECT) $section = null;

                        $s = one('SELECT id, is_active FROM mo_subjects WHERE name = ?', [$subjName]);
                        if (!$s) {
                            $log['subjects'][] = $subjName;
                            if (!$dry) { q('INSERT INTO mo_subjects (name) VALUES (?)', [$subjName]); $s = ['id' => (int)db()->lastInsertId()]; }
                            else $s = ['id' => 0];
                        } elseif (!$dry && !$s['is_active']) {
                            q('UPDATE mo_subjects SET is_active = 1 WHERE id = ?', [$s['id']]);
                        }
                        $log['rows']++; $fileLog['rows']++;

                        /* един ред се записва за всяка избрана професия */
                            ...
                            if (!$replace && (int)$s['id'] > 0) {
                                $dup = one('SELECT id FROM mo_competencies
                                            WHERE subject_id = ? AND grade_level = ? AND (program_id <=> ?)
                                              AND (section <=> ?) AND title = ? AND is_active = 1',
                                           [$s['id'], $level, $progId, $section, mb_substr($title, 0, 600)]);
                                if ($dup) { $log['dupes']++; $fileLog['dupes']++; continue; }
                            }

                            $key = $s['id'] . '-' . $level . '-' . ($progId ?? 0) . '-' . ($section ?? '');

                            /* при „замени“ старите се скриват веднъж за цялото качване,
                               за да не изтрие втори файл това, което първият е записал.
                               Подразделът влиза в ключа: замяната на „Литература“ не
                               бута компетентностите по „Български език“. */
                            if ($replace && !$dry && !isset($wiped[$key])) {
                                q('UPDATE mo_competencies SET is_active = 0
                                   WHERE subject_id = ? AND grade_level = ? AND (program_id <=> ?)
                                     AND (section <=> ?)',
                                  [$s['id'], $level, $progId, $section]);
                                $wiped[$key] = true;
                            }
                            $order[$key] = ($order[$key] ?? 0) + 10;

                            $label = $subjName . ($section ? ' / ' . $section : '')
                                   . ' · ' . $level . ' клас · ' . $tg['label'];
                            $log['pairs'][$label] = ($log['pairs'][$label] ?? 0) + 1;

                            if (!$dry) {
                                q('INSERT INTO mo_competencies (subject_id, section, grade_level, program_id, code, title, source, sort_order)
                                   VALUES (?,?,?,?,?,?,?,?)',
                                  [$s['id'], $section, $level, $progId, $get($r, 'code') ?: null,
                    ...
                    /* грешният файл се пропуска, останалите продължават */
                    $log['failed'][] = ['name' => $uf['name'], 'reason' => $fe->getMessage()];
                }
            }

            if (!$dry) {
                q('INSERT INTO mo_import_log (user_id, filename, rows_ok, rows_error, details) VALUES (?,?,?,?,?)',
                  [$u['id'], implode(', ', array_column($uploaded, 'name')),
                   $log['ok'], $log['skip'] + count($log['failed']), json_encode($log, JSON_UNESCAPED_UNICODE)]);
                db()->commit();
            }
            $report = ['log' => $log, 'dry' => $dry];

            if ($log['failed']) {
                flash('Не бяха приети ' . count($log['failed']) . ' файла: '
                    . e(implode('; ', array_map(static fn($f) => $f['name'] . ' – ' . $f['reason'], $log['failed'])))
                    . '. Останалите са обработени.', 'warn');
            }
        } catch (Throwable $ex) {
    ...
    /* ---------------- ръчно добавяне ---------------- */
    elseif ($act === 'add') {
        [$subj, $autoSec] = normalize_subject(trim((string)($_POST['subject'] ?? '')));
        if ($autoSec !== null) $_POST['section'] = $autoSec;
        ...
        if ($s && trim((string)($_POST['title'] ?? '')) !== '') {
            ...
            if (!in_array($sec, BEL_SECTIONS, true)) $sec = null;
            q('INSERT INTO mo_competencies (subject_id, section, grade_level, program_id, code, title, source, sort_order)
               VALUES (?,?,?,?,?,?,?,?)',
              [$s['id'], $sec, (int)($_POST['grade_level'] ?? 8),
    ...
    elseif ($act === 'edit') {
        ...
        if ($id && $s) {
            $sec = trim((string)($_POST['section'] ?? ''));
            if (!in_array($sec, BEL_SECTIONS, true)) $sec = null;
            q('UPDATE mo_competencies SET subject_id=?, section=?, grade_level=?, program_id=?, code=?, title=?, source=?
               WHERE id=?',
              [$s['id'], $sec, (int)($_POST['grade_level'] ?? 8),
               ($_POST['program_id'] ?? '') !== '' ? (int)$_POST['program_id'] : null,
...
$list = $sid ? all('SELECT k.*, pr.name AS program_name, pr.kind AS program_kind,
                    ...
                    ORDER BY k.grade_level, ' . section_order_sql('k') . ', k.sort_order, k.id', [$sid]) : [];
...
<div class="panel">
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <label>Подраздел – само за „Български език и литература“
      <select name="section">
        <option value="">без подраздел / определя се от файла</option>
        <?php foreach (BEL_SECTIONS as $sec): ?>
          <option value="<?= e($sec) ?>"><?= e($sec) ?></option>
        <?php endforeach; ?>
      </select>
    </label>
    <p class="muted small">Файл, в който предметът е записан като „Литература“ или „Български език“,
       се разпознава сам и отива в БЕЛ като съответния подраздел – менюто важи само когато
       файлът казва просто „Български език и литература“. За другите предмети няма значение.</p>

    <label>Файлове
      <input type="file" name="files[]" id="impFiles" multiple
             accept="<?= class_exists('ZipArchive') ? '.xlsx,.csv' : '.csv' ?>" required>
    </label>
    <p class="muted small" id="fileList">Може да изберете няколко файла наведнъж – важат за
       отметнатите по-горе професии. Ограничения на сървъра: до
       <?= (int)ini_get('max_file_uploads') ?> файла, общо до <?= e(ini_get('post_max_size')) ?>.</p>
...
<div class="panel">
  ...
  <?php if ($L['failed']): ?>
    <div class="flash err">
      <strong>Не бяха приети <?= count($L['failed']) ?> файла:</strong>
      <ul style="margin:.4rem 0 0">
        <?php foreach ($L['failed'] as $f): ?>
          <li><?= e($f['name']) ?> – <?= e($f['reason']) ?></li>
        <?php endforeach; ?>
      </ul>
      Останалите файлове са обработени. Поправете тези и ги качете отново –
      вече заредените редове няма да се дублират.
    </div>
  <?php endif; ?>

  <?php if ($L['files']): ?>
    <table class="grid">
      <thead><tr><th>Файл</th><th>Лист</th><th>Заглавен ред</th><th>Редове</th>
                 <th>Записани</th><th>Пропуснати</th><th>Дубликати</th></tr></thead>
      <tbody>
      <?php foreach ($L['files'] as $f): ?>
        <tr>
          <td><strong><?= e($f['name']) ?></strong></td>
          <td class="small"><?= e($f['sheet'] ?: '—') ?></td>
          <td class="small"><?= (int)$f['head'] ?></td>
          <td><?= (int)$f['rows'] ?></td>
          <td><?= (int)$f['ok'] ?></td>
          <td class="<?= (int)$f['skip'] ? 'danger' : '' ?>"><?= (int)$f['skip'] ?></td>
          <td><?= (int)$f['dupes'] ?></td>
        </tr>
      <?php endforeach; ?>
      </tbody>
  ...
  <?php endif; ?>
  <ul class="small">
    <li>Приети файла: <strong><?= count($L['files']) ?></strong>
        <?php if ($L['failed']): ?><span class="danger">(<?= count($L['failed']) ?> отказани)</span><?php endif; ?></li>
    <li>Редове общо: <strong><?= (int)($L['rows'] ?? 0) ?></strong></li>
...
<div class="panel">
  ...
  <form method="post">
    ...
    <div class="fields">
      ...
      <label>Подраздел (само за БЕЛ)
        <select name="section">
          <option value="">няма</option>
          <?php foreach (BEL_SECTIONS as $sec): ?><option value="<?= e($sec) ?>"><?= e($sec) ?></option><?php endforeach; ?>
        </select>
      </label>
...
<div class="panel">
  ...
  <table class="grid">
    <thead><tr><th>Клас</th><th>Подраздел</th><th>Професия / специалност</th><th>Код</th><th>Компетентност</th>
    ...
    <?php foreach ($list as $c): ?>
          ...
          <form method="post" class="fields">
            ...
            <label>Подраздел (само за БЕЛ)
              <select name="section">
                <option value="">няма</option>
                <?php foreach (BEL_SECTIONS as $sec): ?>
                  <option value="<?= e($sec) ?>" <?= (string)$c['section'] === $sec ? 'selected' : '' ?>><?= e($sec) ?></option>
                <?php endforeach; ?>
              </select>
            </label>
            <label>Професия / специалност
      ...
      <tr class="<?= $c['is_active'] ? '' : 'off' ?>">
        ...
        <td class="small"><?= $c['section'] ? '<span class="badge">' . e($c['section']) . '</span>' : '—' ?></td>
      ...
      <?php endif; ?>
    <?php endforeach; ?>
    <?php if (!$list): ?><tr><td colspan="8" class="muted">Няма въведени компетентности.</td></tr><?php endif; ?>
    </tbody>
  </table>
...
<script>
(function () {
  /* показваме кои файлове са избрани, преди да се качат */
  var fileInput = document.getElementById('impFiles');
  var fileList = document.getElementById('fileList');
  if (fileInput && fileList) {
    var base = fileList.textContent;
    fileInput.addEventListener('change', function () {
      var f = Array.prototype.map.call(fileInput.files, function (x) { return x.name; });
      if (!f.length) { fileList.textContent = base; fileList.classList.remove('picked'); return; }
      fileList.innerHTML = '<strong>Избрани ' + f.length + ' файла:</strong> ' +
        f.map(function (n) {
          return '<span class="file-chip">' + n.replace(/[&<>"]/g, '') + '</span>';
        }).join(' ');
      fileList.classList.add('picked');
    });
  }
