...
$D = summary_data((int)$u['id'], $yid, $term);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'ai') {
        try {
            ...
            $text = Ai::summarize(Ai::buildPrompt(ai_context($D, $u['display_name'], $term)));
    ...
    q('UPDATE mo_summaries SET title=?, summary_text=?, strengths=?, improvements=?, measures=?,
              other=?, notes=?, status=?, deputy_id=?,
              sent_at = CASE WHEN ?="sent" THEN NOW() ELSE sent_at END
       WHERE id=?',
      [$vals['title'], $vals['summary_text'], $vals['strengths'], $vals['improvements'],
       $vals['measures'], $vals['other'], $vals['notes'], $status, $dep, $status, $sid]);
...
$docs = all('SELECT * FROM mo_documents WHERE user_id=? AND year_id=? AND term=? ORDER BY id DESC LIMIT 3',
            [$u['id'], $yid, $term]);
